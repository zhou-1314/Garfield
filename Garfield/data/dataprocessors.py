...
def edge_level_split(
    ...
    data_no_self_loops.edge_index, data_no_self_loops.edge_attr = remove_self_loops(
    ...
    if edge_label_adj is not None:
        ...
        data_no_self_loops.edge_label = torch.tensor(
            [
                (edge_label_adj[edge_index[0].item(), edge_index[1].item()] == 1.0)
                for edge_index in data_no_self_loops.edge_attr
            ]
        ).int()
...
def node_level_split_mask(
    data: Data, val_ratio: float = 0.1, test_ratio: float = 0.0, split_key: str = "x"
) -> Data:
    """
    ...
    random_node_split = RandomNodeSplit(
        num_val=val_ratio, num_test=test_ratio, key=split_key
    )
...
def prepare_data(
    ...
    data_dict["node_masked_data"] = node_level_split_mask(
        data=data_node, val_ratio=node_val_ratio, test_ratio=node_test_ratio
    )
    return data_dict
